...
export const COMPANY_CONTACT_ORDERS_QUERY=`#graphql
query GearGuardContactOrders($id:ID!,$first:Int!,$after:String){companyContact(id:$id){id orders(first:$first,after:$after,sortKey:CREATED_AT,reverse:true){pageInfo{hasNextPage endCursor}nodes{id name createdAt cancelledAt currentSubtotalPriceSet{shopMoney{amount currencyCode}}}}}}`;
